...
(function (window) {
  ...
  // Existing-agreement ha (CSAM3 / CIGL1) come from SfiGrasslandsV4ExistingAgreements and stack
  ...
  function applyExistingAgreementDeductions (code, unit, baseEligible, exclusions) {
    ...
    var allDeductions = []
    if (typeof window.SfiGrasslandsV4ExistingAgreements.getDeductions === 'function') {
      allDeductions = window.SfiGrasslandsV4ExistingAgreements.getDeductions(state.parcelId) || []
    }
  ...
  function getParcelProfile (parcelId, parcel) {
    ...
    if (parcel && Array.isArray(parcel.landCover)) {
      var allocatedCovers = window.SfiGrasslandsV4ParcelReference &&
        typeof window.SfiGrasslandsV4ParcelReference.allocateLandCoverAreas === 'function'
        ? window.SfiGrasslandsV4ParcelReference.allocateLandCoverAreas(parcel.landCover, totalHa)
        : null
  ...
  function formatAvailabilityDeductionLabel (item, featureKey) {
    if (featureKey === 'existingAgreement') {
      ...
      if (window.SfiGrasslandsV4ExistingAgreements &&
          typeof window.SfiGrasslandsV4ExistingAgreements.formatLabel === 'function' &&
          (item.code || item.name)) {
        return window.SfiGrasslandsV4ExistingAgreements.formatLabel(item)
      }
  ...
  function isActionDeductionBreakdownEnabled () {
    if (window.SfiGrasslandsV4FeatureToggles &&
        typeof window.SfiGrasslandsV4FeatureToggles.isToggleEnabled === 'function') {
      return window.SfiGrasslandsV4FeatureToggles.isToggleEnabled('actionDeductions')
    }
    return false
  }
  ...
  window.SfiGrasslandsV4Aac = api
  window.SfiGrasslandsV4ProtectedLand = {
