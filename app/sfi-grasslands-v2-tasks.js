...
var SELECT_LAND_HREF = '/sfi-grasslands-v2/select-land'
...
function ensureTasks (req) {
  ...
  if (!data.sfiGrasslandsV2Tasks || typeof data.sfiGrasslandsV2Tasks !== 'object') {
    data.sfiGrasslandsV2Tasks = Object.assign({}, DEFAULT_TASKS)
  } else {
    Object.keys(DEFAULT_TASKS).forEach(function (taskId) {
      if (!data.sfiGrasslandsV2Tasks[taskId]) {
        data.sfiGrasslandsV2Tasks[taskId] = DEFAULT_TASKS[taskId]
      }
      // Migrate older in-progress values for non-early check tasks to Incomplete
      if (
        IN_PROGRESS_TASKS.indexOf(taskId) === -1 &&
        data.sfiGrasslandsV2Tasks[taskId] === STATUS.IN_PROGRESS
      ) {
...
function getResolvedTaskStates (req) {
  ...
  var checkBusinessDetails = resolveCheckTask(
    businessStored,
    businessCompleted,
    '/sfi-grasslands-v2/check-business-details'
  )
  ...
  if (!section1Complete) {
    ...
    selectLand = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-v2/confirm-land-and-actions'
    }
  } else {
    selectLand = {
  ...
  if (!selectLandCompleted) {
    ...
    checkAnswers = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-v2/check-your-answers'
    }
  } else {
    checkAnswers = {
      key: STATUS.INCOMPLETE,
