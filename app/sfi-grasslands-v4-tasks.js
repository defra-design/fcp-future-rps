...
var SELECT_LAND_HREF = '/sfi-grasslands-v4/select-land'
...
function ensureTasks (req) {
  ...
  if (!data.sfiGrasslandsV4Tasks || typeof data.sfiGrasslandsV4Tasks !== 'object') {
    data.sfiGrasslandsV4Tasks = Object.assign({}, DEFAULT_TASKS)
  } else {
    Object.keys(DEFAULT_TASKS).forEach(function (taskId) {
      if (!data.sfiGrasslandsV4Tasks[taskId]) {
        data.sfiGrasslandsV4Tasks[taskId] = DEFAULT_TASKS[taskId]
      }
      // Migrate older in-progress values for non-early check tasks to Incomplete
      if (
        IN_PROGRESS_TASKS.indexOf(taskId) === -1 &&
        data.sfiGrasslandsV4Tasks[taskId] === STATUS.IN_PROGRESS
      ) {
        data.sfiGrasslandsV4Tasks[taskId] = STATUS.INCOMPLETE
      }
    })
    // Opening management control used to store Incomplete; treat that as In progress
    if (data.sfiGrasslandsV4Tasks.confirmEligible === STATUS.INCOMPLETE) {
      data.sfiGrasslandsV4Tasks.confirmEligible = STATUS.IN_PROGRESS
    }
  }

  return data.sfiGrasslandsV4Tasks
}
...
function getResolvedTaskStates (req) {
  ...
  var checkLandDetails = resolveCheckTask(
    ...
    '/sfi-grasslands-v4/check-land-details',
    businessCompleted
  )
  ...
  if (!section1Complete) {
    ...
    selectLand = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-v4/confirm-land-and-actions'
    }
  } else {
    selectLand = {
  ...
  if (!selectLandCompleted) {
    ...
    checkAnswers = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-v4/check-your-answers'
    }
  } else {
    checkAnswers = {
      key: STATUS.INCOMPLETE,
  ...
  if (!checkAnswersCompleted) {
    ...
    submitApplication = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-v4/submit-application'
    }
  } else {
    submitApplication = {
      key: STATUS.INCOMPLETE,
