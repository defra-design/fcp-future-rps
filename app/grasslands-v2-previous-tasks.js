...
var SELECT_LAND_HREF = '/grasslands-v1-archive/select-land'
...
function ensureTasks (req) {
  ...
  if (!data.grasslandsV2PreviousTasks || typeof data.grasslandsV2PreviousTasks !== 'object') {
    data.grasslandsV2PreviousTasks = Object.assign({}, DEFAULT_TASKS)
  } else {
    Object.keys(DEFAULT_TASKS).forEach(function (taskId) {
      if (!data.grasslandsV2PreviousTasks[taskId]) {
        data.grasslandsV2PreviousTasks[taskId] = DEFAULT_TASKS[taskId]
      }
      // Migrate older in-progress values for non-early check tasks to Incomplete
      if (
        IN_PROGRESS_TASKS.indexOf(taskId) === -1 &&
        data.grasslandsV2PreviousTasks[taskId] === STATUS.IN_PROGRESS
      ) {
        data.grasslandsV2PreviousTasks[taskId] = STATUS.INCOMPLETE
      }
    })
  }

  return data.grasslandsV2PreviousTasks
}
...
function getResolvedTaskStates (req) {
  ...
  var checkBusinessDetails = resolveCheckTask(
    businessStored,
    businessCompleted,
    '/grasslands-v1-archive/check-business-details'
  )

  var checkLandDetails = resolveCheckTask(
    landDetailsStored,
    landDetailsCompleted,
    '/grasslands-v1-archive/check-land-details'
  )
  ...
  if (eligibleCompleted) {
    confirmEligible = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/grasslands-v1-archive/management-control'
    }
  ...
  if (!section1Complete) {
    ...
    selectLand = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/grasslands-v1-archive/confirm-land-and-actions'
    }
  } else {
    selectLand = {
      key: STATUS.INCOMPLETE,
  ...
  if (!selectLandCompleted) {
    ...
    checkAnswers = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/grasslands-v1-archive/check-your-answers'
    }
  } else {
    checkAnswers = {
      key: STATUS.INCOMPLETE,
  ...
  if (!checkAnswersCompleted) {
    ...
    submitApplication = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/grasslands-v1-archive/submit-application'
    }
  } else {
    submitApplication = {
      key: STATUS.INCOMPLETE,
