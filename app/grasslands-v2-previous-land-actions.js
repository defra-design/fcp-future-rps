...
var parcelReference = require('./grasslands-v2-previous-parcel-reference')
...
function getApplicationParcels (req) {
  ...
  var parcels = parseJson(data.previousApplicationParcels, [])
  if (!Array.isArray(parcels)) {
    parcels = []
  }
  data.previousApplicationParcels = parcels
  return parcels
}

function getDraftParcel (req) {
  var data = getSessionData(req)
  return parseJson(data.previousDraftLandParcel, null)
}

function setDraftParcel (req, parcel) {
  var data = getSessionData(req)
  data.previousDraftLandParcel = parcel || null
  return data.previousDraftLandParcel
}

function getDraftActions (req) {
  var data = getSessionData(req)
  var actions = parseJson(data.previousDraftLandActions, [])
...
function setDraftActions (req, actions) {
  var data = getSessionData(req)
  data.previousDraftLandActions = Array.isArray(actions) ? actions : []
  return data.previousDraftLandActions
}

function clearDraft (req) {
  var data = getSessionData(req)
  delete data.previousDraftLandParcel
  delete data.previousDraftLandActions
  delete data.previousDraftParcelId
}
...
function upsertApplicationParcel (req, parcelEntry) {
  ...
  getSessionData(req).previousApplicationParcels = parcels
  return parcels
}
...
function syncParcelSelectionsData (req) {
  getSessionData(req).previousParcelSelectionsData = buildParcelSelectionsDataFromApplication(req)
}
...
function loadParcelIntoDraftForEdit (req, parcelId) {
  ...
  getSessionData(req).previousApplicationParcels = parcels.filter(function (entry) {
    return entry.parcelId !== parcelId
  })
  syncParcelSelectionsData(req)
  return true
