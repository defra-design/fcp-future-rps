...
function ensureTasks (req) {
  ...
  if (!data.sfiGrasslandsDevReadyTasks || typeof data.sfiGrasslandsDevReadyTasks !== 'object') {
    data.sfiGrasslandsDevReadyTasks = Object.assign({}, DEFAULT_TASKS)
  } else {
    Object.keys(DEFAULT_TASKS).forEach(function (taskId) {
      if (!data.sfiGrasslandsDevReadyTasks[taskId]) {
        data.sfiGrasslandsDevReadyTasks[taskId] = DEFAULT_TASKS[taskId]
      }
      // Migrate older in-progress values for non-early check tasks to Incomplete
      if (
        IN_PROGRESS_TASKS.indexOf(taskId) === -1 &&
        data.sfiGrasslandsDevReadyTasks[taskId] === STATUS.IN_PROGRESS
      ) {
        data.sfiGrasslandsDevReadyTasks[taskId] = STATUS.INCOMPLETE
      }
    })
    // Opening management control used to store Incomplete; treat that as In progress
    if (data.sfiGrasslandsDevReadyTasks.confirmEligible === STATUS.INCOMPLETE) {
      data.sfiGrasslandsDevReadyTasks.confirmEligible = STATUS.IN_PROGRESS
    }
  }

  return data.sfiGrasslandsDevReadyTasks
}
...
function getResolvedTaskStates (req) {
  ...
  var checkLandDetails = resolveCheckTask(
    ...
    '/sfi-grasslands-dev-ready/check-land-details'
  )
  ...
  if (!section1Complete) {
    ...
    selectLand = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-dev-ready/confirm-land-and-actions'
    }
  } else {
    selectLand = {
  ...
  if (!selectLandCompleted) {
    ...
    checkAnswers = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-dev-ready/check-your-answers'
    }
  } else {
    checkAnswers = {
      key: STATUS.INCOMPLETE,
  ...
  if (!checkAnswersCompleted) {
    ...
    submitApplication = {
      key: STATUS.COMPLETED,
      status: statusViewCompleted(),
      href: '/sfi-grasslands-dev-ready/submit-application'
    }
  } else {
    submitApplication = {
      key: STATUS.INCOMPLETE,
